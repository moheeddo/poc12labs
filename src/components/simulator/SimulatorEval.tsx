    ...
    <div className="max-w-[1440px] mx-auto px-6 py-6 space-y-6 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          {/* 종합 점수 */}
          <div className="bg-surface-800 border border-coral-500/30 rounded-xl p-4 text-center">
              ...
              {scores.map((s) => (
                <div key={s.key} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-surface-700 rounded-full h-1.5">
                    <div
                      className="h-1.5 rounded-full bg-coral-500 transition-all duration-500"
                      style={{ width: `${s.score}%` }}
                    />
                  </div>
                  <span className="text-xs font-mono text-slate-300 w-8 text-right">{s.score}</span>
                </div>
              ))}
            </div>
          </div>
