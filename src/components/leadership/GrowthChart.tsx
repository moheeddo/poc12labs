...
function trendBadgeColor(direction: CompetencyTrend["direction"]) {
  if (direction === "improving") return "bg-emerald-50 border-emerald-200 text-emerald-700";
  if (direction === "declining") return "bg-red-50 border-red-200 text-red-700";
...
function CustomTooltip({ active, payload, label }: {
  ...
  return (
    <div className="rounded-xl border border-slate-200/60 bg-white/95 backdrop-blur-sm p-3 shadow-lg min-w-[160px]">
            ...
            {trends.map((trend) => {
              ...
              return (
                ...
                >
                  ...
                  {isPlateau && (
                    <span className="inline-flex items-center gap-0.5 ml-0.5 px-1.5 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200 text-[10px]">
                      <AlertTriangle className="w-2.5 h-2.5" />
                      정체
                    </span>
                  )}
                  {isBreakthrough && (
                    <span className="inline-flex items-center gap-0.5 ml-0.5 px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px]">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {Object.entries(projectedMap).map(([key, score]) => {
              ...
              return (
                <div
                  key={key}
                  className="rounded-lg border border-slate-200/60 bg-slate-50 p-3 text-center"
                >
