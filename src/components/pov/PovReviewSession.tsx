...
import DebriefingGuide, {
  DebriefingGuideToggleButton,
...
export default function PovReviewSession({
  ...
  // ── SBI 디브리핑 가이드 상태 ──
  const [showDebriefingGuide, setShowDebriefingGuide] = useState(false);
  const [structuredFeedbacks, setStructuredFeedbacks] = useState<StructuredFeedbackItem[]>([]);
  ...
  // ── SBI 피드백 추가 ──
  const addStructuredFeedback = useCallback((feedback: StructuredFeedbackItem) => {
    setStructuredFeedbacks((prev) => {
      // 동일 stepId가 있으면 덮어쓰기
      const filtered = prev.filter((f) => f.stepId !== feedback.stepId);
      return [...filtered, feedback];
    });
    // 논의완료로 자동 등록
    setDiscussedSteps((prev) => {
      const next = new Set(prev);
      next.add(feedback.stepId);
      return next;
    });
  }, []);
    ...
    <div className="max-w-[1440px] mx-auto space-y-4 animate-fade-in-up">
      ...
      <div className="bg-white border border-amber-500/20 rounded-xl p-4">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-3">
          ...
          {/* SBI 디브리핑 가이드 토글 버튼 */}
          <DebriefingGuideToggleButton
            onClick={() => setShowDebriefingGuide((v) => !v)}
            isOpen={showDebriefingGuide}
            discussedCount={structuredFeedbacks.length}
      ...
      <div className="bg-white border border-slate-200 rounded-xl px-5 py-4 flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        <div className="flex items-center gap-2 sm:ml-auto flex-wrap">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-sm text-slate-700 border border-slate-200 transition-colors"
          >
            <FileText className="w-3.5 h-3.5" /> 훈련 기록서 생성
          </button>
          <button
            onClick={() => {
      ...
      {showHandoff && (
        <HandoffDocument
          report={report}
          procedure={procedure}
          instructorName={report.evaluatorName}
          traineeName={report.traineeInfo}
          feedbacks={structuredFeedbacks}
          overrides={overrides}
